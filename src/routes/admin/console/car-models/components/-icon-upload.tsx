...
const MAX_ICON_SIZE_BYTES = 512 * 1024 // 512 KB (half MB)
...
// Helper function to validate and create icon
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      ...
      if (blob.size > MAX_ICON_SIZE_BYTES) {
        reject(new Error(`Icon size exceeds ${MAX_ICON_SIZE_BYTES / 1024 / 1024}MB limit`))
        return
      }
...
export function IconUpload({
  ...
  const handleFileSelect = (file: File | null) => {
    ...
    if (file.size > MAX_ICON_SIZE_BYTES) {
      setUploadError(`Icon size must be no larger than ${MAX_ICON_SIZE_BYTES / 1024 / 1024}MB (you selected ${(file.size / 1024 / 1024).toFixed(2)}MB)`)
      return
    }
  ...
  const handleCropConfirm = async () => {
    ...
    try {
      // Create cropped blob
      const croppedBlob = await createIconImage(imageSrc, croppedAreaPixels)
    ...
    return (
      <div className="space-y-3">
        <div className="border-2 border-dashed border-muted-foreground/30 rounded-lg p-6 hover:border-muted-foreground/50 transition-colors">
          <div className="flex flex-col items-center justify-center gap-3">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-muted-foreground">PNG only (Max 512 KB)</p>
            </div>
            <Input
              type="file"
        ...
        <FieldDescription className="text-center">
          Icon will be cropped to a square (128x128 pixels)
        </FieldDescription>
      </div>
    )
  }
